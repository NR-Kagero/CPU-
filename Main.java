        ...
        for (int i = 0; i < n; i++) {
            ...
            for (int j = 0; j < p.size(); j++) {
                if (p.get(j).get_priority_number() == minp) {
                    p.get(j).setTermination(p.get(j).get_burst_Time() + g.get(g.size() - 1).getTermination() + context);
                    g.add(p.get(j));
                    p.remove(j);
                    break;
                }
...
//            }
            ...
            if (ready_p.get(0).non_pree) {
                ...
                calculate_factors(last_arrival_time, Remaining_Burst_Time);
                for (process process : all_processes) {
                    System.out.println(process.getname()+": "+process.getAGAT_Factor());
                }
...
class Main {
    ...
    public static void main(String[] args) {
        ...
        priority_method agat = new priority_method(Totalprocesses,context_switching);
        ArrayList<process> l = agat.solve();
        print.print(l);
    }
}
